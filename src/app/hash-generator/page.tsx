...
export default function HashGenerator() {
  ...
  const generateHashes = async () => {
    ...
    results.push({ algorithm: "SHA-1", hash: await shaHash("SHA-1", input) });
    results.push({ algorithm: "SHA-256", hash: await shaHash("SHA-256", input) });
    results.push({ algorithm: "SHA-512", hash: await shaHash("SHA-512", input) });
    setHashes(results);
  };
  ...
  return (
    <ToolPageWrapper title="Hash Generator" description="Generate MD5, SHA-1, SHA-256, and SHA-512 hashes from text.">
      <div className="tool-card p-6 mb-6">
        <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Input Text</label>
        <textarea
      ...
      {hashes.length > 0 && (
        <div className="space-y-3">
          {hashes.map((h) => (
            <div key={h.algorithm} className="tool-card p-4">
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm font-semibold text-[var(--text-primary)]">{h.algorithm}</span>
                <button className="btn-secondary text-xs px-2 py-1" onClick={() => copyHash(h.algorithm, h.hash)}>
                  {copied === h.algorithm ? "Copied!" : "Copy"}
                </button>
              </div>
              <p className="font-mono text-xs text-[var(--text-secondary)] break-all">{h.hash}</p>
            </div>
          ))}
        </div>
      )}
